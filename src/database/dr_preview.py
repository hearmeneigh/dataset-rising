...
import argparse
import os
from typing import Generator, List, Union, Dict, Tuple, Optional
...
def get_args():
    ...
    parser.add_argument('--filter', metavar='TAG', type=str, help='Filter results by additional tag(s)', required=False, action='append', default=[])
...
def save_results_to_jsonl(path: str, results: Union[Dict[str, Tuple[List[SelectedSample], Optional[int], Optional[str]]], Generator[SelectedSample, None, None]], file_subtitle: str):
...
def save_results_to_html(
    path: str,
    results: Union[Dict[str, Tuple[List[SelectedSample], Optional[int], Optional[str]]], Generator[SelectedSample, None, None]],
    template: Template,
    selector_file: str,
    file_subtitle: str,
):
    ...
    if isinstance(results, Generator):
        ...
        items.sort(key=lambda x: int(x[1][1]), reverse=True)
                ...
                'tags': [
                    {
                        'title': tag_item[0],
                        'images': tag_item[1][0],
                        'total_count': tag_item[1][1],
                        'url': tag_item[1][2],
                    } for tag_item in page_chunk],
                'pagination': {
...
def main():
    ...
    if args.aggregate:
        if (args.filter is not None) and (len(args.filter) > 0):
            raise Exception('Cannot use --filter with --aggregate')
        ...
        result = selector.sample_selectors(samples=args.limit, formats=args.image_format, filters=args.filter)
        progress = Progress('Generating preview', 'samples')
