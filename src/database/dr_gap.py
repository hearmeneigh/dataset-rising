...
def save_results_to_jsonl(path: str, results: List[Tuple[str, int, List[PostEntity], str]], file_subtitle: str):
...
def save_results_to_html(
    path: str,
    results: List[Tuple[str, int, List[PostEntity], str]],
    template: Template,
    category: str,
    file_subtitle: str,
):
    ...
    results.sort(key=lambda x: int(x[1][1]), reverse=True)
...
def sample_posts(tag_name: str, limit: int, db: Database, image_format: List[str]) -> Tuple[str, int, List[PostEntity], str]:
    post_count_result = list(db['posts'].aggregate(pipeline=[
        {'$match': {'tags': tag_name, 'origin_format': {'$in': image_format}, 'image_url': {'$exists': True}}},
        {'$count': 'total'}
    ]))
    ...
    return tag_name, post_count, [PostEntity(post) for post in posts], tag_url
...
def main():
    ...
    def get_post_url_tpl(post: PostEntity) -> Optional[str]:
